...
- Google reverse image search returns results for Masterboice High Crosses, one of which is Muirdach's High Cross

- A Daily Beast article on [The Ancient Art of the Celtic People](https://www.thedailybeast.com/the-ancient-art-of-the-celtic-people-photos/) has a photo with the same cross and also showing the background crosses on the left and attributes this to Muirdach's High Cross
...
- Further examination shows that a few variables are passed directly to the HTML template:
    ...
    - `spells_cast`
    - `turns_survived`
    - technically, `battle_duration` is also passed directly but is coverted to float so that won't work
...
With this we can pass our popen file read to any of the other variables that are passed to the template and the flag will returned in the HTML output e.g.
